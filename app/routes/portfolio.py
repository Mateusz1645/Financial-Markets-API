...
from services.market_data_services import (
    get_current_price_from_yfinance,
    get_forex_rate,
)
...
def upload_portfolio(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    if not (file.filename.endswith(".xlsx") or file.filename.endswith(".csv")):
        raise HTTPException(
            status_code=400, detail="Unsupported file type. Use Excel or CSV."
        )
    try:
    ...
    for _, row in df.iterrows():
        ...
        inflation_first_year = (
            (
                round(inflation_first_year / 100, 4)
                if inflation_first_year and inflation_first_year >= 1
                else inflation_first_year
            )
            if inflation_first_year is not None
            else None
        )
        type_ = type_.upper()
...
def add_asset(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="Date must be in format DD.MM.YYYY HH:MM"
        )
        ...
        result.append(
            {
                ...
                if a.amount is not None and pd.notna(a.amount)
                else 0,
                "transaction_price": float(a.transaction_price)
                if a.transaction_price is not None and pd.notna(a.transaction_price)
                else 0,
                "currency": a.currency,
                "currency_transaction": a.currency_transaction,
                "type": a.type_,
                "coupon_rate": float(a.coupon_rate)
                if a.coupon_rate is not None and pd.notna(a.coupon_rate)
                else None,
                "inflation_first_year": float(a.inflation_first_year)
                if a.inflation_first_year is not None
                and pd.notna(a.inflation_first_year)
                else None,
            }
        )
...
def calculate_asset_value(
    ...
    if id:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Date format should be YYYY-MM-DD"
            )
        ...
        if not asset:
            raise HTTPException(
                status_code=404, detail=f"Asset {isin} not found for given date {date}"
            )
    else:
        raise HTTPException(
            status_code=400, detail="Provide either `id` or both `isin` and `date`"
        )
    ...
    if date_to_calculate == "today":
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail="date_to_calculate format should be YYYY-MM-DD or 'today'",
            )
    ...
    if asset_date > calc_date:
        raise HTTPException(
            status_code=400,
            detail=(
                f"Cannot calculate value for {calc_date}. "
                f"Asset was acquired on {asset_date}."
            ),
        )
    ...
    try:
        if asset.type_.upper() == "BOND":
            ...
            if not isfinite(value):
                raise HTTPException(
                    status_code=500,
                    detail=f"Calculated bond value is not finite: {value}",
                )
            if asset.amount == 0:
            ...
            if len(symbols) == 1:
                price_data = get_current_price_from_yfinance(
                    symbols[0], target_date=date_to_calculate
                )
                ...
                if str(currency) != str(asset.currency):
                    raise HTTPException(
                        status_code=500,
                        detail=f"There are different currencies in equities for: {asset.isin, asset.id}. Input currency = {asset.currency}, yfinance currency = {currency}",
                    )

                forex_rate = get_forex_rate(
                    db, currency, asset.currency_transaction, calc_date
                )
                value = round(value_per_unit * asset.amount * forex_rate, 4)
            else:
                HTTPException(
                    status_code=500,
                    detail=f"More symbols than one or no symbol for {asset.isin}: {symbols}",
                )
        else:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error calculating value for {asset.isin}: {str(e)}",
        )
