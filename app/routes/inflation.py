...
def add_inflation(
    ...
    if inflation:
        raise HTTPException(
            status_code=400,
            detail=f"Inflation for month {month}, year {year} already exists: {inflation.value}",
        )
    if value is None:
        max_retries = 5
        last_exception = None
    ...
    inflation = Inflation(month=month, year=year, value=value)
...
def delete_inflation(inflation_id: int, db: Session = Depends(get_db)):
    ...
    return {
        "status": "success",
        "message": f"Inflation with id {inflation_id} deleted year: {inflation.year}, month: {inflation.month}, value: {inflation.value}",
    }
