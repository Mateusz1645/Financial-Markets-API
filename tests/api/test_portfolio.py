...
def test_calc_current_value_bonds(client):
    ...
    for bond_data in BONDS_TO_CALCULATE.values():
        ...
        assert response_add.status_code == 200, f"Status: {response_add.status_code}, Respons: {response_add.text}"
        ...
        assert response_calc.status_code == 200, f"Status: {response_calc.status_code}, Respons: {response_calc.text}"

        data = response_calc.json()[0]

        assert data["isin"] == bond_data["isin"], f"Wrong isin got: {data['isin']} expected: {bond_data['isin']}"
        assert data["value"] >= bond_data["transaction_price"], f"Current value is smaller than transaction price: {data['value']} < {bond_data['transaction_price']}"
        assert data["value_in_currency_per_unit"] > 0, f"value_in_currency_per_unit = {data['value_in_currency_per_unit']} < 0"
        assert data["amount"] == bond_data["amount"], f"Amount before not equal after: before - {data['amount']}, after - {bond_data['amount']}"
