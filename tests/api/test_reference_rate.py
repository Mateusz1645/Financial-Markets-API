...
def test_add_reference_rate(client, db_session):
    response = client.post("/reference_rate/add", params={
        "month": 1,
        "year": 2099,
        "value": 0.2
    })
    ...
    assert data["message"] == "Reference Rate added successfully", f"Unexpected message: {data}"
...
def test_delete_reference_rate(client, db_session):
    ...
    response = client.delete("/reference_rate/delete", params={"reference_rate_id": record.id})
    assert response.status_code == 200, f"Expected 200, got {response.status_code}. Response: {response.text}"
